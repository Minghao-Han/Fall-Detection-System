...
vector<DetectBox> FCOSDetector::operator()(const cv::Mat &input_mat)
{
...
cv::Mat FCOSDetector::preprocess(const cv::Mat &img, hbDNNTensor &input_tensor)
{
    ...
    cv::resize(img, img, cv::Size(512, 512),0.0,0.0,cv::INTER_AREA);

    // Convert the resized image to NV12 format
    return bgr2nv12_opencv(img);
}

inline cv::Mat distance2bbox(const cv::Mat& points, const cv::Mat& distance) {
