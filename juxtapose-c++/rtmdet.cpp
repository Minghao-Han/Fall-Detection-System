...
vector<DetectBox> RTMDet::operator()(const cv::Mat& input_mat,const float threshold)
{
	...
	for (int i = 0; i < num_dets; ++i)
	{
        ...
        float score=det_result[i * reshap_dims + 4];
        if(score<threshold)
            continue;
		DetectBox temp_box;
		temp_box.left = det_result[i * reshap_dims];
		temp_box.top = det_result[i * reshap_dims + 1];
		temp_box.right = det_result[i * reshap_dims + 2];
		temp_box.bottom = det_result[i * reshap_dims + 3];
		temp_box.score = score;
