...
#define LSTM_MODEL_PATH "mymodel/rnn_model.bin"

static int8_t THREAD_NUM_MAX = 5; 

static bool detect_continue=false;
static std::queue<Results> bbox_kpts_results;
...
fall_detector_t *fall_detector_init(sem_t *fall_sem,frame_buf_t *clip){
    ...
    fall_detector->lstm_analyzer = new LSTMDetector(LSTM_MODEL_PATH);
...
void *fall_detector_start(void *args){
    ...
    for (int i = 0; i < THREAD_NUM_MAX; ++i) {
        bbox_kpts_threads.emplace_back(get_bbox_kpts_task, fall_detector->bbox_kpts_detector, &detect_continue);
    }
    BYTETracker *byte_tracker = (BYTETracker *)fall_detector->byte_tracker;
    LSTMDetector *lstm_analyzer = (LSTMDetector *)fall_detector->lstm_analyzer;
    vector<DetectBox> bbox;
    vector<int> ids;
    do{
        Results results = bbox_kpts_results.front();
        auto &bbox = results.bboxes;
        if (bbox.size() == 0)continue;
        auto &kptss = results.kptss;
        ids=byte_tracker->update(bbox);
        for(auto id:ids){
            lstm_analyzer->inputss[id].push({kptss[id],bbox[id]});
        }
